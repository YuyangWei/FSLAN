...
class Trainer:
    def __init__(self, data_loaders, dataset, parameter):
        ...
        self.ae_weight = parameter['ae_weight']
        self.load_embed()
        ...
        self.co_entities = self.count_imply(self.e1_rele2, self.num_symbols)

        self.metaR = MetaR(dataset, parameter, self.num_symbols, self.co_entities, embed = self.symbol2vec)
    ...
    def count_imply(self, graph, num_symbols):		
        co_entities = np.zeros((num_symbols+1, num_symbols+1), dtype=np.dtype('float32'))
        freq_entities = defaultdict(int)
		
        for entity in graph:
            entities_list = list(set([neighbor[1] for neighbor in graph[entity]]))
            for n_i in range(len(entities_list)):
                e_i = entities_list[n_i]
                freq_entities[e_i] += 1
                for n_j in range(n_i+1, len(entities_list)): 
                    e_j = entities_list[n_j]
                    co_entities[e_i][e_j] += 1
                    co_entities[e_j][e_i] += 1		

        for i in range(num_symbols+1):
            if freq_entities[i] == 0:
                continue
            else:
                co_entities[i] = (co_entities[i] * 1.0) / freq_entities[i]
        print('finish calculating co entities')
        list_co_entities = co_entities.tolist()
        temp_list = []
        for value in list_co_entities:
            if value == 0.0:
                continue
            else:
    ...
    def task2target(self, task):
        target_pairs = np.zeros((self.batch_size, self.few, 2), dtype=np.dtype('int'))
        support = task[0]

        for i in range(len(support)):
            for j in range(len(support[0])):
                target_pairs[i, j, 0] = self.symbol2id[support[i][j][0]]
                target_pairs[i, j, 1] = self.symbol2id[support[i][j][2]]
        
        target_pairs = torch.tensor(target_pairs).cuda()
    ...
    def do_one_step(self, task, iseval=False, curr_rel='', istest=False):
        ...
        target_pairs = self.task2target(task)
        ...
        if not iseval:
            ...
            p_score, n_score, ae_loss = self.metaR(task, target_pairs, iseval, curr_rel, support_meta, istest)
            y = torch.Tensor([1]).to(self.device)
            loss = self.metaR.loss_func(p_score, n_score, y) + ae_loss * self.ae_weight
            loss.backward()
            self.optimizer.step()
        elif curr_rel != '':
            p_score, n_score, _ = self.metaR(task, target_pairs, iseval, curr_rel, support_meta, istest)
            y = torch.Tensor([1]).to(self.device)
